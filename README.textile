...
* You are making double pressing &lt;Esc&gt; in normal or command modes.
...
If you want to use another keyboard layout as default, edit the plugin file (~/.vim/plugin/KeyboardLayoutSwitcher.vim) by changing this line:
@let g:defaultInputSourceIndex = 0@
to:
@let g:defaultInputSourceIndex = n " n is index of wanted keyboard layout@
...
При этом каждый таб имеет помнит свою раскладку.
...
Как уже говорилось выше, из коробки в качестве раскладки по умолчанию используется английская раскладка США, потому в обычном случае настройка не требуется.

Если вы всё же хотите использовать другую раскладку, то отредактируйте файл плагина (~/.vim/plugin/KeyboardLayoutSwitcher.vim), изменив строчку:
